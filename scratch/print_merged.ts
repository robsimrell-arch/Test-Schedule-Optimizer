...
async function run() {
  // Use shifts = 1, workDays = 5 as default for standard view
  const shifts = 1;
  const workDays = 5;
  ...
  function runSimulation(ignoreBOM: boolean, unconstrainedStartTimes?: any) {
    ...
    for (const passIgnoreBOM of passes) {
      if (passIgnoreBOM && ignoreBOM) break; // If this run is already unconstrained, don't run pass 2
  ...
  // Group and merge tasks as done in routes.ts
